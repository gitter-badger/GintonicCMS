...
class WebsocketComponent extends Component
{
    ...
    /**
     * List of websocket objects attached 
     *
     * @var array
     */
    protected $_websocketInstances = [];
    ...
    {
        $config += ['actions' => [], 'websockets' => []];
        $config['actions'] = $this->normalizeArray($config['actions']);
        $config['websockets'] = $this->normalizeArray($config['websockets']);
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->_controller->dispatchComponents['Websocket'] = true;

        $this->_loadWebsockets();
    ...
    public function execute($controllerAction = null, $args = [])
    {
        ...
        $view = null;
        $websocketAction = $this->action($action);
        if (method_exists($websocketAction, 'view')) {
            $view = $websocketAction->view();
        }
     ...
     */
    public function websocket($name)
    {
        return $this->_loadWebsocket($name);
    }
     ...
     * @return void
     */
    public function addWebsocket($name, $className = null, $config = [])
    {
        ...
        $name = Inflector::variable($name);
        $this->config(sprintf('websockets.%s', $name), compact('className') + $config);
    }
     ...
     * @return bool
     */
    public function removeWebsocket($name)
    {
        $websockets = $this->config('websockets');
        if (!array_key_exists($name, $websockets)) {
            return false;
        }

        if (isset($this->_websocketInstances[$name])) {
            $this->_eventManager->off($this->_websocketInstances[$name]);
            unset($this->_websocketInstances[$name]);
        }

        unset($websockets[$name]);
        $this->_config['websockets'] = $websockets;
    }
     ...
     * @return void
     */
    protected function _loadWebsockets()
    {
        foreach (array_keys($this->config('websockets')) as $name) {
            $this->_loadWebsocket($name);
        }
    }

    /**
     * Load a single Websocket Listner class attached to this component.
     *
     * @param string $name Name
     * @return \Crud\Listener\BaseListener
     ...
     */
    protected function _loadWebsocket($name)
    {
        if (!isset($this->_websocketInstances[$name])) {
            $config = $this->config('websockets.' . $name);

            if (empty($config)) {
                throw new ListenerNotConfiguredException(sprintf('Websocket "%s" is not configured', $name));
            }

            $className = App::classname($config['className'], 'Websocket', 'Websocket');
            if (empty($className)) {
                throw new MissingListenerException('Could not find websocket class: ' . $config['className']);
            }

            $this->_websocketInstances[$name] = new $className($this->_controller);
            unset($config['className']);
            $this->_websocketInstances[$name]->config($config);

            $this->_eventManager->on($this->_websocketInstances[$name]);

            if (is_callable([$this->_websocketInstances[$name], 'setup'])) {
                $this->_websocketInstances[$name]->setup();
            }
        }

        return $this->_websocketInstances[$name];
    }

    /**
     * TODO: this action needs to dispatch the event to the right websocket
     *
     * @param string $name The controller action name.
     * @return \Crud\Action\BaseAction
